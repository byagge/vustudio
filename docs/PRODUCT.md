...
JSX: `photoshop/render.jsx`, версия `2026-09-13.2`. На VPS копировать этот файл и **перезапустить worker**. Оборот: даты 10/11 только Python на JPG (`back_jpg_dates.py`), без сценового оверлея и без замазывания бланка. Портрет в Photo — cover без полей.
