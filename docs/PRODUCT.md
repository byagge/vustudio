...
- Отрисовка в мокап: рука, фон 1–10 или свой загруженный фон, портрет в smart object `Photo`.
- Telegram-бот: загрузка фона при создании ВУ; «Сменить фон» на готовой задаче → пересборка.
...
JSX: `photoshop/render.jsx`, версия `2026-09-14.5`. На VPS копировать этот файл и **перезапустить worker**. Оборот: даты 10/11 только Python на JPG (`back_jpg_dates.py`), без сценового оверлея и без замазывания бланка. Портрет в Photo — cover без полей.
