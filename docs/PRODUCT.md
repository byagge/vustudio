...
JSX: `photoshop/render.jsx`, версия `2026-09-13.1`. На VPS копировать этот файл и **перезапустить worker**. Даты 10/11 на обороте рисует Python на JPG (`back_jpg_dates.py`) по найденному бланку — строки B, B1, M по центрам ячеек таблицы. Портрет в Photo — cover без полей.
