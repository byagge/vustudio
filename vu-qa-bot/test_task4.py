...
class TestTask4Integration(unittest.TestCase):
    ...
    def test_full_job_json_scene_block(self):
        ...
        self.assertIn("Back", job["scene"]["card_smart_objects"])
        self.assertIn("output_jpg_back", job)
        self.assertIn("Слой 0 копия", job["scene"]["card_wrappers"])
        self.assertIn("fonts", job)
        # серия/номер: шрифт и для blank-плейсхолдеров («04»), и для hand («77»)
        font_layers = (job.get("fonts") or {}).get("by_layer_name") or {}
        self.assertEqual(font_layers.get("04"), "z_nomer0")
        self.assertEqual(font_layers.get("77"), "z_nomer0")
        self.assertEqual(font_layers.get("04 76 656492"), "znomer")
        self.assertEqual(font_layers.get("77 35 739911"), "znomer")
        self.assertTrue(job.get("back_jpg_draw_dates"))
    ...
    def test_jsx_closes_by_name_not_stale_handle(self):
        ...
        self.assertIn("2026-09-16.1", jsx)
        ...
        self.assertIn("resolveBackTable", jsx)
        self.assertIn("fallbackHandCardBox", jsx)
        self.assertIn("scaleActiveLayerFit", jsx)
        self.assertIn("fillDocPaperGray", jsx)
        self.assertIn("vu_photo_paper", jsx)
        self.assertIn("BACK_ROW_FRAC", jsx)
        self.assertIn("back table dates named=", jsx)
        self.assertIn("applyBackTableDates", jsx)
        self.assertIn("stampBackTableDates", jsx)
        self.assertIn("applyCustomBackground", jsx)
        self.assertIn("custom_background_path", jsx)
        self.assertIn("overlaySceneBackDates", jsx)
        self.assertIn("scene dates overlaid=", jsx)
        self.assertIn("inferCardFromTallBox", jsx)
        self.assertIn("_stampDates", jsx)
        self.assertIn("isLandscapeCardDoc", jsx)
        self.assertIn("date-like layers filled", jsx)
        self.assertIn("text-group slots=", jsx)
        self.assertIn("hide group '", jsx)
        self.assertIn("keep group 'Text' (front SO)", jsx)
        self.assertIn("exportBackJpeg", jsx)
        self.assertIn("keep Text visible (replace-only)", jsx)
        self.assertIn('toggleNamedLayers(doc, "Text", true)', jsx)
        self.assertIn("show back Text groups", jsx)
        self.assertIn("card side enter SO", jsx)
        self.assertIn("back jpeg ok", jsx)
        self.assertIn("variant original showWall=", jsx)
        self.assertIn("studio bg hidden for original", jsx)
        ...
        self.assertIn("!isBackCardName(layer.name)", jsx)
