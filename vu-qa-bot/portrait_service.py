...
def generate_ai_portrait(
    ...
    *,
    ...
    log.info(
        "portrait generate job=%s provider=%s force=%s",
        jid,
        cfg.resolved_provider(),
        force,
    )

    if cfg.cache_enabled and not force:
        cached = _cache_path(fields)
        if cached.is_file():
            shutil.copy2(cached, out)
            log.info("portrait cache hit job=%s", jid)
...
def resolve_portrait(task: RenderTask) -> str | None:
    ...
    cfg = PortraitSettings.from_env()
    log.info(
        "portrait resolve job=%s generate=%s path=%s provider=%s",
        task.job_id,
        opts.generate_portrait,
        opts.portrait_path,
        cfg.resolved_provider(),
    )
    ...
    if not result.ok:
        ...
        return None
    log.info("portrait generated job=%s source=%s path=%s", task.job_id, result.source, result.path_str)
...
def portrait_status_label(opts) -> str:
    path = getattr(opts, "portrait_path", None)
    if path:
